...
class AccountList:
    ...
    def register(self,name,lastname,email,user_name,password,user_phone,Role):
        ...
        regex = re.compile('[@_!#$%^&*()<>?/\|}{~:!]')
        check_number = r'^[0-9]'
        if (name=='' or lastname == ''or email == '' or user_name == '' or password == '' or user_phone == ''):
            return "press any values"
        if re.fullmatch(pattern,email) and regex.search(password) == None and (re.match(check_number,user_phone )and len(user_phone)==10):
            ...
            if( Role == "User"):
                ...
                new_user = Owner(name,lastname,email,user_name,password,user_phone)
                print(new_user)
